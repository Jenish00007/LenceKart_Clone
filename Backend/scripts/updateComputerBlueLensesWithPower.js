...
const blueLensProducts = Array.from({ length: 15 }, (_, index) => {
  ...
  return {
    name: `Computer Blue Light Glasses with Power ${productNumber}`,
    imageTsrc: blueLensImages[imageIndex],
    caption: `Best Selling Blue Light Protection with Power ${productNumber}`,
    productRefLink: `computer-blue-light-power-${productNumber}`,
    price: 1999,
    mPrice: 3499,
    productId: `BLUP${String(productNumber).padStart(3, '0')}`,
...
const updateComputerBlueLensesWithPower = async () => {
  try {
    // Connect to MongoDB
    ...
    // Delete all existing computer blue light glasses with power
    await ProductModel.deleteMany({ 
      productType: 'COMPUTER_BLU_LENSES',
      powerType: 'SINGLE_VISION'
    });
    console.log('Deleted existing computer blue light glasses with power');

    // Insert new computer blue light glasses with power
    await ProductModel.insertMany(blueLensProducts);
    console.log('Inserted new computer blue light glasses with power');

    console.log('Computer blue light glasses with power update completed successfully');
  } catch (error) {
    console.error('Error updating computer blue light glasses with power:', error);
  } finally {
...
updateComputerBlueLensesWithPower(); 