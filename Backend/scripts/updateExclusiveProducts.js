...
const exclusiveProducts = Array.from({ length: 10 }, (_, index) => {
  const imageIndex = index % exclusiveImages.length;
  ...
  return {
    name: `Exclusive ${style} ${shape} ${productType.split('_').join(' ')} ${productNumber}`,
    imageTsrc: exclusiveImages[imageIndex],
    caption: `Limited Edition ${style} ${shape} ${productType.split('_').join(' ')}`,
    productRefLink: `exclusive-${productType.toLowerCase()}-${productNumber}`,
    price: 4999,
    mPrice: 6999,
    productId: `EXC${String(productNumber).padStart(3, '0')}`,
...
const updateExclusiveProducts = async () => {
  try {
    // Connect to MongoDB
    ...
    // Check existing exclusive products
    const existingCount = await ProductModel.countDocuments({ isExclusive: true });
    console.log(`Found ${existingCount} existing exclusive products`);

    // Delete all existing exclusive products
    console.log('Deleting existing exclusive products...');
    const deleteResult = await ProductModel.deleteMany({ 
      productId: { $regex: '^EXC' }
    });
    console.log(`Deleted ${deleteResult.deletedCount} existing exclusive products`);

    // Insert new exclusive products
    console.log('Inserting new exclusive products...');
    const insertResult = await ProductModel.insertMany(exclusiveProducts);
    console.log(`Successfully inserted ${insertResult.length} new exclusive products`);

    // Verify the insertion
    const finalCount = await ProductModel.countDocuments({ isExclusive: true });
    console.log(`Final count of exclusive products in database: ${finalCount}`);

    console.log('Exclusive products update completed successfully');
  } catch (error) {
